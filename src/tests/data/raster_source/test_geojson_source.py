...
class TestGeoJSONSource(unittest.TestCase):
    def setUp(self):
        ...
        class_ids = [1]
        geojson = boxes_to_geojson(boxes, class_ids, self.crs_transformer, class_map)
        str_to_file(json.dumps(geojson), self.uri)

        self.source = GeoJSONSource(self.uri, self.extent, self.crs_transformer)
    ...
    def test_get_chip(self):
        ...
        expected_chip = 2 * np.ones((20, 20, 1))
        expected_chip[5:15, 5:15, 0] = 1
